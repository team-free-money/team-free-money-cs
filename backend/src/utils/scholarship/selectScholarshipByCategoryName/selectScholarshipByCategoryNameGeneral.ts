...
export async function selectScholarshipByCategoryNameGeneraL(categoryName: string) : Promise<Scholarship[]> {
    try {
        const mySqlConnection = await connect();
        const mySqlQuery = "SELECT BIN_TO_UUID(scholarshipId) as scholarshipId, scholarshipAmount, scholarshipCriteria, scholarshipDeadline, scholarshipDescription, scholarshipLink, scholarshipName FROM scholarship INNER JOIN scholarshipCategory ON scholarship.scholarshipId = scholarshipCategory.scholarshipCategoryScholarshipId INNER JOIN category ON scholarshipCategory.scholarshipCategoryCategoryId = category.categoryId WHERE categoryName = 'General'";
